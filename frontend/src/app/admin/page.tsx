...
export default function AdminDashboard() {
  ...
  const [newUser, setNewUser] = useState({ email: "", name: "", password: "", role: "teacher" as UserRole });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-text-primary tracking-tight">System Administration</h1>
            <p className="text-text-secondary">Monitor platform activity and manage users.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            { label: "Active Polls", value: metrics?.activePolls ?? "...", icon: Activity, color: "text-accent" },
            { label: "Total Responses", value: metrics?.totalResponses ?? "...", icon: BarChart3, color: "text-secondary" },
            { label: "Response Rate (7d)", value: metrics?.recentResponses ?? "...", icon: CheckCircle2, color: "text-warning" },
          ].map((stat, idx) => (
            <div key={idx} className="glass-card p-6 rounded-2xl animate-fade-in" style={{ animationDelay: `${idx * 100}ms` }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-glass-border">
                    {users.map((u) => (
                      <tr key={u.id} className="hover:bg-card-hover/20 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                            u.role === "admin" ? "bg-primary-muted text-primary" :
                            u.role === "teacher" ? "bg-accent-muted text-accent" :
                        ...
                        <td className="px-6 py-4 text-right space-x-2">
                          {u.role === "student" && (
                            <button onClick={() => promoteUser(u.id)} title="Promote to Teacher" className="p-1.5 text-text-muted hover:text-accent transition-colors">
                              <ArrowUpCircle className="w-4 h-4" />
                            </button>
                          )}
                          {u.id !== user.id && (
                            <button onClick={() => deleteUser(u.id)} title="Delete User" className="p-1.5 text-text-muted hover:text-danger transition-colors">
          ...
          <div className="space-y-6">
            <div className="glass-card rounded-2xl h-[calc(100vh-300px)] flex flex-col animate-fade-in" style={{ animationDelay: "200ms" }}>
              <div className="px-6 py-4 border-b border-glass-border flex items-center gap-2 bg-card/30">
                <ShieldAlert className="w-5 h-5 text-warning" />
                <h2 className="text-lg font-bold text-text-primary">Audit Logs</h2>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {logs.map((log) => (
                  <div key={log.id} className="p-3 rounded-xl border border-glass-border bg-surface-elevated/40 text-xs">
                    ...
                    <p className="text-text-secondary leading-relaxed">
                      <span className="font-medium text-text-primary">{log.admin.name}</span> acted on {log.targetType} ({log.targetId?.slice(0,8)})
                    </p>
                  </div>
                ))}
              </div>
          ...
          <div className="max-w-md w-full glass-card p-8 rounded-3xl animate-scale-in">
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div>
                ...
                <input required type="text" value={newUser.name} onChange={(e) => setNewUser({...newUser, name: e.target.value})} className="input-field" placeholder="John Doe" />
              </div>
              <div>
              ...
              <div>
                ...
                <input required type="password" value={newUser.password} onChange={(e) => setNewUser({...newUser, password: e.target.value})} className="input-field" placeholder="••••••••" />
              </div>
              <div>
              ...
              {formError && <p className="text-xs text-danger bg-danger-muted/20 p-3 rounded-lg border border-danger/20">{formError}</p>}
